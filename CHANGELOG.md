...
- **Changed: suggested entity ids now carry a short tag of the config entry**, so the entities of two entries can no longer end up with the same suggested id. Version 1.10 put the radius into the ids of a radius entry's filter entities, matching the availability sensor, which keeps a 10 km and a 15 km entry apart — but the flow allows two entries at different locations with the same radius, and two favorite sites may share a name, and in both cases the entities suggested identical ids again (`select.charging_stations_status_15km` twice), which Home Assistant resolves by appending `_2` to whichever entry loads second; an automation copied between them silently targets the wrong one. Every suggested id now has the last four characters of the entry's id, lower-cased, after its radius or name: `sensor.charging_stations_available_15km_7f3a`, `select.charging_stations_status_15km_7f3a`, `sensor.charging_station_favorite_migros_aarau_7f3a_power_kw`, `sensor.charging_station_favorite_location_migros_aarau_7f3a_connector_1_status`. The tag stays fixed for the life of the entry; the README entity tables show the pattern. A suggested id only applies when an entity is first created, so **the entities of existing entries keep the ids they have**, unique ids are unchanged, and there is nothing to do on upgrade.
- Documented in the code: when a site leaves the radius (or a favorite site's connector disappears from the source), its map marker or sensors are removed from the state machine but their registry rows are kept on purpose — the site usually comes back on a later refresh or filter change and gets its entity id, history, and whatever you changed on it (name, area, visibility) back. Only the per-plug-type sensors of a radius entry drop their registry row, because there the plug type was deselected deliberately in the Configure dialog. No change in behavior.
...
- **New: a dashboard strategy** as the replacement. A strategy is a recipe Home Assistant renders in the browser: it stores nothing, overwrites nothing, and reflects your current setup on every page load — add a favorite and its section appears, delete one and it is gone, with no stale card left behind. It ships inside the card file, so no extra resource is needed. Setup is in the README; in short, a new dashboard with:
  ...
  It renders a full-screen map of the stations in range (markers labeled with live availability), then the radius search with its filter entities, then one section per favorite with the bundled card and that favorite's headline sensors.
...
- Documented: the ich-tanke-strom.ch GeoServer endpoint is plain HTTP because the operator offers no TLS listener on any port. The data is public and read-only, and the requests carry no credentials — see the note in `const.py`.
